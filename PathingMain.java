...
public class PathingMain extends PApplet
{
    ...
    */
    private boolean DFS(Point pos, GridValues[][] grid, List<Point> path)
    {
        ...
        {
            //check if the passed position is the goal and set flagPath to true and return later if so
            if (grid[pos.y][pos.x] == GridValues.GOAL)
                flagPath = true;

                //set this value as searched and recurse on the nodes surrounding
            else{
                grid[pos.y][pos.x] = GridValues.SEARCHED;
                ...
                flagPath =  DFS(rightN, grid, path) ||
                        DFS(leftN, grid, path) ||
                        DFS(downN, grid, path)||
                        DFS(upN, grid, path);
            }
        }
        //once goal is reached, return recurses, adding path steps to path list
